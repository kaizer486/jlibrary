...
<div class="mb-6">
    <div class="flex justify-between items-center flex-wrap gap-4">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">Manage inspiring quotes displayed on the dashboard</p>
        </div>
        <a href="{{ route('super-admin.quotes.create') }}" class="bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 transition flex items-center gap-2">
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-xl p-4 shadow-sm border-l-4 border-purple-500">
        ...
        <p class="text-2xl font-bold text-gray-800">{{ \App\Models\Quote::count() }}</p>
    </div>
    <div class="bg-white rounded-xl p-4 shadow-sm border-l-4 border-green-500">
        <p class="text-gray-500 text-sm">Active Quotes</p>
        <p class="text-2xl font-bold text-green-600">{{ \App\Models\Quote::where('status', 'active')->count() }}</p>
    </div>
    <div class="bg-white rounded-xl p-4 shadow-sm border-l-4 border-blue-500">
        <p class="text-gray-500 text-sm">Total Saves</p>
        <p class="text-2xl font-bold text-blue-600">{{ \App\Models\Quote::sum('saves_count') }}</p>
    </div>
    <div class="bg-white rounded-xl p-4 shadow-sm border-l-4 border-yellow-500">
        <p class="text-gray-500 text-sm">Total Shares</p>
        <p class="text-2xl font-bold text-yellow-600">{{ \App\Models\Quote::sum('shares_count') }}</p>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm p-4 mb-6">
    <form method="GET" class="flex flex-col md:flex-row gap-4">
        ...
        <div>
            ...
            <a href="{{ route('super-admin.quotes.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 ml-2">Clear</a>
        </div>
    </form>
</div>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex gap-2">
                            <a href="{{ route('super-admin.quotes.edit', $quote) }}" class="text-blue-600 hover:text-blue-800" title="Edit">
                                <i class="ti ti-edit"></i>
                            </a>
                            <a href="{{ route('super-admin.quotes.analytics', $quote) }}" class="text-green-600 hover:text-green-800" title="Analytics">
                                <i class="ti ti-chart-bar"></i>
                            </a>
                            <form method="POST" action="{{ route('super-admin.quotes.destroy', $quote) }}" 
