...
<div class="container mx-auto px-4 py-6">
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">Update the slide content</p>
        </div>
    </div>

    <div class="bg-white rounded-xl shadow-sm p-6 max-w-4xl">
        <form action="{{ route('super-admin.hero-slides.update', $heroSlide) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="subtitle" rows="3" 
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition">{{ old('subtitle', $heroSlide->subtitle) }}</textarea>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="slide_type" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition">
                            @foreach($slideTypes as $value => $label)
                                <option value="{{ $value }}" {{ old('slide_type', $heroSlide->slide_type) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="text" name="cta_text" value="{{ old('cta_text', $heroSlide->cta_text) }}" 
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">CTA URL</label>
                            <input type="text" name="cta_url" value="{{ old('cta_url', $heroSlide->cta_url) }}" 
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="slide_duration" value="{{ old('slide_duration', $heroSlide->slide_duration ?? 5) }}" min="2" max="30"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Order</label>
                            <input type="number" name="order" value="{{ old('order', $heroSlide->order) }}" 
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition">
                        </div>
                    </div>
