...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Edit Founder</h1>
            <p class="text-gray-500 text-sm mt-1">Update founder details</p>
        </div>
    </div>

    <!-- Form -->
    <div class="bg-white rounded-xl shadow-sm p-6 max-w-3xl">
        <form action="{{ route('super-admin.founders.update', $founder) }}" method="POST" enctype="multipart/form-data" id="founderForm">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <textarea name="bio" id="bio" rows="5"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition">{{ old('bio', $founder->bio) }}</textarea>
            ...
            @if($founder->photo)
                <div class="mb-4">
                    ...
                    <div class="flex items-center gap-4 p-3 rounded-lg bg-gray-50 border border-gray-200">
                        <img src="{{ url('media/' . $founder->photo) }}" alt="{{ $founder->name }}" 
                             class="w-16 h-20 rounded-lg object-cover border-2 border-purple-100" style="aspect-ratio: 4/5;">
                        <div>
                            <p class="text-sm text-gray-700 font-medium">{{ $founder->name }}</p>
                            <p class="text-xs text-gray-500">Current ratio: <span class="text-purple-600 font-semibold">4:5</span></p>
                        </div>
                    </div>
                </div>
            @endif

            <!-- New Photo with Cropper -->
            <div class="mb-4">
                <label for="photo" class="block text-sm font-medium text-gray-700 mb-1">
                    {{ $founder->photo ? 'Replace Photo' : 'Profile Photo' }}
                </label>
                ...
                <button type="button" id="pick-photo-btn" 
                    ...
                    <i class="ti ti-upload"></i> {{ $founder->photo ? 'Choose New Photo' : 'Choose Photo' }}
                </button>
                
                <p class="text-xs text-gray-500 mt-2">
                    <i class="ti ti-info-circle"></i> 
                    Crop to <strong>4:5 portrait ratio</strong>. All founder photos must match this ratio.
                </p>
                @error('photo')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
                ...
                <div id="cropper-container" class="mt-4 hidden">
                    <div class="border-2 border-dashed border-purple-200 rounded-xl p-4 bg-purple-50/50">
                        ...
                        <p class="text-xs text-gray-400 mt-2 text-center">
                            Drag to move • Corner handles to resize • Fixed 4:5 ratio
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" name="twitter" id="twitter" 
                               ...
                               value="{{ old('twitter', $founder->social_links['twitter'] ?? '') }}" placeholder="https://x.com/username">
                    </div>
                    <div>
                        <label for="instagram" class="text-xs text-gray-500">Instagram</label>
                        <input type="text" name="instagram" id="instagram" 
                               class="w-full px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition text-sm"
                               value="{{ old('instagram', $founder->social_links['instagram'] ?? '') }}" placeholder="https://instagram.com/username">
                    </div>
                    <div>
                        <label for="facebook" class="text-xs text-gray-500">Facebook</label>
                        <input type="text" name="facebook" id="facebook" 
                               class="w-full px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition text-sm"
                               value="{{ old('facebook', $founder->social_links['facebook'] ?? '') }}" placeholder="https://facebook.com/username">
                    </div>
                    <div>
                        <label for="tiktok" class="text-xs text-gray-500">TikTok</label>
                        <input type="text" name="tiktok" id="tiktok" 
                               class="w-full px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition text-sm"
                               value="{{ old('tiktok', $founder->social_links['tiktok'] ?? '') }}" placeholder="https://tiktok.com/@username">
                    </div>
                    <div>
                        <label for="whatsapp" class="text-xs text-gray-500">WhatsApp Channel</label>
                        <input type="text" name="whatsapp" id="whatsapp" 
                               class="w-full px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition text-sm"
                               value="{{ old('whatsapp', $founder->social_links['whatsapp'] ?? '') }}" placeholder="https://whatsapp.com/channel/...">
                    </div>
                    <div>
                        <label for="youtube" class="text-xs text-gray-500">YouTube</label>
                        <input type="text" name="youtube" id="youtube" 
                               class="w-full px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition text-sm"
                               value="{{ old('youtube', $founder->social_links['youtube'] ?? '') }}" placeholder="https://youtube.com/@channel">
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    founderForm.addEventListener('submit', function(e) {
        if (!cropper || !photoInput.files.length) return;
