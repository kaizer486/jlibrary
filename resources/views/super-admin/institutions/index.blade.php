...
<div class="mb-6">
    <div class="flex justify-between items-center flex-wrap gap-4">
        ...
        <a href="{{ route('super-admin.institutions.create') }}" class="bg-gradient-to-r from-green-600 to-emerald-600 text-white px-5 py-2.5 rounded-xl hover:shadow-lg transition flex items-center gap-2 font-semibold">
...
<div class="bg-white rounded-xl shadow-sm p-4 mb-6">
    <form method="GET" action="{{ route('super-admin.institutions.index') }}" class="flex flex-col md:flex-row gap-4">
        ...
        <div>
            <a href="{{ route('super-admin.institutions.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 transition inline-block">Clear</a>
        </div>
    </form>
</div>
                    ...
                    <td class="px-6 py-4">
                        <span class="px-2 py-1 rounded-full text-xs font-semibold 
                            {{ $inst->subscription_tier === 'premium' ? 'bg-purple-100 text-purple-700' : 
                               ($inst->subscription_tier === 'enterprise' ? 'bg-gold-100 text-gold-700' : 'bg-gray-100 text-gray-700') }}">
                            {{ ucfirst($inst->subscription_tier) }}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-sm">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('super-admin.institutions.show', $inst) }}" class="text-blue-600 hover:text-blue-800" title="View">
                                <i class="ti ti-eye"></i>
                            </a>
                            <a href="{{ route('super-admin.institutions.edit', $inst) }}" class="text-green-600 hover:text-green-800" title="Edit">
                                <i class="ti ti-edit"></i>
                            </a>
                            @if($inst->status === 'pending')
                                <form method="POST" action="{{ route('super-admin.institutions.approve', $inst) }}" class="inline">
                                    @csrf
                                    <button type="submit" class="text-green-600 hover:text-green-800" title="Approve">
                                        <i class="ti ti-check"></i>
                                    </button>
                                </form>
                                <form method="POST" action="{{ route('super-admin.institutions.reject', $inst) }}" class="inline">
                                    @csrf
                            ...
                            @endif
                            @if($inst->status === 'approved')
                                <form method="POST" action="{{ route('super-admin.institutions.suspend', $inst) }}" class="inline">
                                    @csrf
                            ...
                            @if($inst->users_count == 0)
                                <form method="POST" action="{{ route('super-admin.institutions.destroy', $inst) }}" class="inline" onsubmit="return confirm('Delete {{ $inst->name }} permanently?')">
                                    @csrf
                                    @method('DELETE')
