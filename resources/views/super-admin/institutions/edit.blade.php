...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
        ...
        <form method="POST" action="{{ route('super-admin.institutions.update', $institution) }}" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="type" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl bg-white">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address" rows="2" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl">{{ old('address', $institution->address) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="status" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl bg-white">
                ...
                <div>
                    ...
                    <select name="subscription_tier" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl bg-white">
            ...
            <div class="mt-6 p-4 bg-gray-50 rounded-xl">
                <h3 class="font-semibold text-gray-800 mb-3">📊 Institution Statistics</h3>
                <div class="grid grid-cols-3 gap-4 text-center">
                    <div>
                        <p class="text-2xl font-bold text-purple-600">{{ $institution->users()->count() }}</p>
                        <p class="text-xs text-gray-500">Total Users</p>
                    </div>
                    <div>
                        <p class="text-2xl font-bold text-blue-600">{{ $institution->books()->count() }}</p>
            ...
            <div class="flex gap-3 mt-8 pt-6 border-t">
                <button type="submit" class="flex-1 bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-3 rounded-xl hover:shadow-lg transition font-semibold">
                    <i class="ti ti-device-floppy"></i> Save Changes
                </button>
                <a href="{{ route('super-admin.institutions.index') }}" class="px-6 py-3 border border-gray-300 rounded-xl hover:bg-gray-50 transition text-center">
                    Cancel
                </a>
            </div>
        </form>
    ...
    <div class="mt-6 bg-red-50 rounded-2xl border border-red-200 overflow-hidden">
        ...
        <div class="p-6 flex items-center justify-between">
            ...
            <form method="POST" action="{{ route('super-admin.institutions.destroy', $institution) }}" onsubmit="return confirm('Delete {{ $institution->name }} permanently?')">
                @csrf
                @method('DELETE')
