...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
            <p class="text-slate-400 text-sm">Organize your library shelves and locations</p>
        </div>
        <div class="flex gap-3">
            <a href="{{ route('librarian.shelves.create') }}" class="btn-library">
                <i class="ti ti-plus"></i> Add New Shelf
            </a>
        </div>
    </div>
    ...
    <div class="bg-slate-900 border border-slate-700 rounded-xl p-4 mb-6">
        <form method="GET" class="flex flex-wrap gap-3">
            ...
            <a href="{{ route('librarian.shelves.index') }}" class="bg-slate-800 text-slate-400 px-4 py-2 rounded-lg hover:bg-slate-700 transition border border-slate-700">
                <i class="ti ti-x"></i> Clear
            </a>
        </form>
    </div>

    <!-- Shelves Grid - Dark Glassmorphism -->
    @if($shelves->count() > 0)
                    ...
                    <div class="p-4">
                        ...
                        @if($shelf->getFullLocationAttribute() && $shelf->getFullLocationAttribute() !== ' |  |  | ')
                            <div class="mt-2 flex flex-wrap gap-1">
                                ...
                                @endif
                                @if($shelf->column)
                                    <span class="text-xs bg-slate-800 text-slate-300 px-2 py-0.5 rounded-full">📐 {{ $shelf->column }}</span>
                                @endif
                            </div>
                        @endif
                        ...
                        <div class="mt-3 flex items-center justify-between text-sm">
                            <span class="text-slate-300">
                                <i class="ti ti-books text-purple-400"></i> {{ $shelf->books()->count() }} books
                            </span>
                            <span class="text-slate-300">
                        ...
                        <div class="mt-3 flex gap-2 pt-3 border-t border-slate-700">
                            <a href="{{ route('librarian.shelves.show', $shelf) }}" class="text-purple-400 hover:text-purple-300 text-sm font-medium transition">
                                <i class="ti ti-eye"></i> View
                            </a>
                            <a href="{{ route('librarian.shelves.edit', $shelf) }}" class="text-blue-400 hover:text-blue-300 text-sm font-medium transition">
                                <i class="ti ti-edit"></i> Edit
                            </a>
                            <form method="POST" action="{{ route('librarian.shelves.destroy', $shelf) }}" 
                                  onsubmit="return confirm('Delete this shelf? This will remove all books from it.')" class="inline">
                                @csrf
                                @method('DELETE')
    ...
    @else
        <div class="bg-slate-900 border border-slate-700 rounded-xl p-12 text-center">
            <i class="ti ti-layout-grid text-6xl text-slate-600 mb-4 block"></i>
            <h3 class="text-xl font-semibold text-white mb-2">No Shelves Found</h3>
            <p class="text-slate-400">Start organizing your library by creating shelves.</p>
            <a href="{{ route('librarian.shelves.create') }}" class="inline-block mt-4 btn-library">
