...
<div class="max-w-5xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm overflow-hidden mb-6">
        <div class="bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-6">
            <div class="flex items-center gap-6">
                ...
                <div class="ml-auto">
                    <a href="{{ route('admin.users.edit', $user) }}" class="bg-white/20 hover:bg-white/30 text-white px-4 py-2 rounded-xl transition text-sm flex items-center gap-2">
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden mt-6">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <h2 class="font-semibold text-gray-800 flex items-center gap-2">
                <i class="ti ti-info-circle text-indigo-600"></i> Account Information
            </h2>
        </div>
        <div class="p-6">
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="mt-1 capitalize text-gray-700">{{ $user->role }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-400 uppercase tracking-wide">Institution</p>
                    <p class="mt-1 text-gray-700">{{ $user->institution->name ?? 'Not assigned' }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-400 uppercase tracking-wide">Member Since</p>
                    <p class="mt-1 text-gray-700">{{ $user->created_at->format('F d, Y') }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-400 uppercase tracking-wide">Last Updated</p>
                    <p class="mt-1 text-gray-700">{{ $user->updated_at->diffForHumans() }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden mt-6">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <h2 class="font-semibold text-gray-800 flex items-center gap-2">
                <i class="ti ti-books text-indigo-600"></i> Recently Added Books
            </h2>
        </div>
        <div class="p-6">
