...
<div class="bg-white rounded-xl shadow-sm p-4 mb-6">
    <form method="GET" class="flex flex-col md:flex-row gap-4">
        ...
        <div>
            <a href="{{ route('admin.applications.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg">Clear</a>
        </div>
    </form>
</div>
    ...
    <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200">
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4">{!! $app->type_label !!}</td>
                <td class="px-6 py-4">{!! $app->status_badge !!}</td>
                <td class="px-6 py-4 text-sm text-gray-600">{{ $app->created_at->format('M d, Y') }}</td>
                <td class="px-6 py-4">
                    <a href="{{ route('admin.applications.show', $app) }}" class="text-purple-600 hover:text-purple-800">Review</a>
                </td>
            </tr>
            @endforeach
        </tbody>
